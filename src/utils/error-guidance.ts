...
export function buildStructuredErrorPayload(
  ...
  if (error instanceof AsrError) {
    ...
    > = {
      ...
      ASR_FAKE_IP_DNS: {
        en: "Standard Fake-IP DNS was detected for Bilibili audio media.",
        zh: "检测到 Bilibili 音频媒体域名使用了标准 Fake-IP DNS。",
        stepsEn: [
          "Your proxy DNS returned 198.18.0.0/15 Fake-IP placeholders, not the real public Bilibili audio CDN addresses. Exact addresses can vary by user, domain, cache, and time.",
          "The server stopped the download to prevent access to local, private, or special-use addresses. This is normally not a Cookie, ASR model, BVID, or ordinary temporary playback failure.",
          "Option 1 (recommended): keep TUN and rule mode. In the current active config, add exactly `+.bilivideo.com` and `+.bilivideo.cn` to `fake-ip-filter`, save and reload the config or restart the proxy kernel, then retry ASR.",
          "Option 1 affects only those domains: they return a real IP while other TUN behavior and routing rules remain unchanged.",
          "Option 2: switch to `redir-host` or an equivalent real public IP DNS mode. This changes the proxy client's DNS behavior; apply and reload the change, then retry ASR.",
          "Option 3: leave the network unchanged and use Human Subtitle, Bilibili AI Subtitle, or the video description. Local ASR Transcript will be unavailable.",
          "The AI agent must list these choices and wait for the user's explicit choice. Do not automatically close TUN, edit the proxy config, use public DoH to bypass the user's DNS policy, or blindly retry with unchanged settings.",
          "Do not allowlist 198.18.0.0/15 in the MCP server.",
        ],
        stepsZh: [
          "你的代理 DNS 返回了 198.18.0.0/15 Fake-IP 占位地址，而不是真实的 Bilibili 音频 CDN 公网地址。具体地址会因用户、域名、缓存和时间不同而变化。",
          "服务已停止下载，以避免访问本地、私有或特殊用途地址。这通常不是 Cookie、ASR 模型、BVID 或普通的临时播放故障。",
          "方案一（推荐）：保留 TUN 和规则模式。在当前生效的配置中，把精确规则 `+.bilivideo.com` 和 `+.bilivideo.cn` 加入 `fake-ip-filter`，保存并重新加载配置或重启代理内核，然后重试 ASR。",
          "方案一仅让这些域名返回真实 IP，其他 TUN 行为和路由规则保持不变。",
          "方案二：切换到 `redir-host` 或等效的真实公网 IP DNS 模式。这会改变代理客户端的 DNS 行为；应用并重新加载设置后，再重试 ASR。",
          "方案三：不改变网络，改用人工字幕、Bilibili AI 字幕或视频简介；但无法获得本地 ASR 转录。",
          "AI Agent 必须列出这些方案并等待用户明确选择。不要自动关闭 TUN、修改代理配置、使用公共 DoH 绕过用户的 DNS 策略，或在设置不变时盲目重试。",
