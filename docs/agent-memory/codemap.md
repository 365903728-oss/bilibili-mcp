...
- `src/bilibili/creator-content.ts`: authenticated Creator Content Discovery for one caller-selected `mid`. Versioned canonical base64url cursors bind mid, section, page, and optional Collection/Series container ID before network access. The module normalizes the bounded overview and video catalog plus separate combined-page Collection/Series container lists and selected-container member pages; Series membership revalidates metadata ownership before archives, every page is capped at 20, upstream order/overlapping Memberships are preserved, and malformed target rows contribute to `skipped_count`.
...
- `src/utils/validation.ts`: BV, language, detail-level, comment/search limits, sort, query, max_matches, context_segments, Favorites cursor (type/length/base64url charset), and Creator Content input (mid/section/cursor/container identity) validation.
...
- `tests/bilibili-creator-content.test.ts`: Creator Content Discovery cursor and credential gates; exact overview/video/container/member endpoint parameters and request counts; profile/catalog/Collection/Series/member normalization; combined-page continuation, selected-container cursor binding, overlapping BVID Memberships, skipped rows, ownership mismatch, byte/resource limits, structured/text parity, and error integrity.
